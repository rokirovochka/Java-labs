...
public class Multiplication implements Command {
    ...
    public Multiplication(String args) {
        ...
        if (cnt != 0)
            throw new WrongNumberOfArguments("wrong number of arguments in command Multiplication.\nexpected 0. found " + cnt);
    }

    @Override
    public void execute(DataStorage data) {
        if (data.getOperands().size() < 2)
            throw new NotEnoughOperands("not enough operands in stack for Multiplication command: expected 2. found " + data.getOperands().size());
        ...
        if (operand1 == null || operand2 == null)
            throw new IncorrectArgument("incorrect argument for Multiplication command");
        double result = operand1 * operand2;
        if (!Double.isFinite(result))
            throw new IncorrectResult("incorrect result if execute Multiplication command with args" + operand1 + " " + operand2);
